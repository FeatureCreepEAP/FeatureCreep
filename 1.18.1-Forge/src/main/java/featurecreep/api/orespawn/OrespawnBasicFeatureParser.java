...
import featurecreep.legacy.BiomeGen;
...
public class OrespawnBasicFeatureParser {
	  ...
	  static List<PlacedFeature> placed = new ArrayList<PlacedFeature>();
...
if (node.get("enabled").asBoolean() == true)
{
	...
	// final RegistryEntry<PlacedFeature> ORE_PLACED = PlacedFeatures.register(name+"_placed", ORE_CONFIG, modifiersWithCount(node.get("parameters").get("frequency").asInt(), HeightRangePlacementModifier.uniform(YOffset.fixed(node.get("parameters").get("minHeight").asInt()), YOffset.fixed(node.get("parameters").get("maxHeight").asInt()))));// YOffset.getBottom is for bottom
	 ...
	 final ConfiguredFeature<?, ?> ORE_CONFIG = Feature.ORE
				.configure(new OreFeatureConfig(
						RULE,
						newBlock.getDefaultState(),
						node.get("parameters").get("size").asInt())); // Vein size
				final PlacedFeature ORE_PLACED = ORE_CONFIG.withPlacement(
						net.minecraft.world.gen.decorator.CountPlacementModifier.of(node.get("parameters").get("frequency").asInt()), // number of veins per chunk
						net.minecraft.world.gen.decorator.SquarePlacementModifier.of(), // spreading horizontally
						net.minecraft.world.gen.decorator.HeightRangePlacementModifier.uniform(YOffset.fixed(node.get("parameters").get("minHeight").asInt()), YOffset.fixed(node.get("parameters").get("maxHeight").asInt()))); // height
