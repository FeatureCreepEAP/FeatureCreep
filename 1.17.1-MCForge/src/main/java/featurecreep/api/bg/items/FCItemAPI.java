...
@Override
 public default void registerModels() {
    ...
    node.get("textures").get("layer0").set(getModId() + ":items/" + getUnlocName());

    System.out.print(node.toJSONString(false));

    try {

      File myObj = new File(featurecreep.api.bg.PackLoader.fc_pack_location + "/assets/" + getModId() + "/models/item/" + getUnlocName() + ".json");
