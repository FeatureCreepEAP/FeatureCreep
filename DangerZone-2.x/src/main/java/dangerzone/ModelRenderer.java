...
public class ModelRenderer {
    ...
    public void connectPart(ModelRenderer inpart){
    	...
    	//allow multiple connections as secondary
    	//if(inpart.connected)return;
    	
    	//can't connect to slave part either!
    	if(connected)return;
    ...
    private void compile_connected_part(ModelRenderer inpart){
    	...
    	while(rotateAngleX < 0)rotateAngleX += Math.PI*2;
    	while(rotateAngleX >= Math.PI*2)rotateAngleX -= Math.PI*2;
    	while(rotateAngleY < 0)rotateAngleY += Math.PI*2;
    	while(rotateAngleY >= Math.PI*2)rotateAngleY -= Math.PI*2;
    	
		cosrx = (float) Math.cos(Math.PI*2 - rotateAngleX);
		sinrx = (float) Math.sin(Math.PI*2 - rotateAngleX);
		cosry = (float) Math.cos(Math.PI*2 - rotateAngleY);
		sinry = (float) Math.sin(Math.PI*2 - rotateAngleY);	
		cosrz = (float) Math.cos(rotateAngleZ);
		sinrz = (float) Math.sin(rotateAngleZ);
		
		//System.out.printf("cosrx,  sinrx == %f, %f\n", cosrx, sinrx);
		//System.out.printf("cosry,  sinry == %f, %f\n", cosry, sinry);
		//System.out.printf("cosrz,  sinrz == %f, %f\n", cosrz, sinrz);
		...
		drawoffx = newx;
		drawoffy = newy;
		//System.out.printf("1    drawoff x,  y, z = %f, %f, %f\n", drawoffx, drawoffy, drawoffz);
		// rot y
		newx = drawoffx*cosry - drawoffz*sinry;
		newz = drawoffz*cosry + drawoffx*sinry;
		drawoffx = newx;
		drawoffz = newz;
		//System.out.printf("2    drawoff x,  y, z = %f, %f, %f @ %f\n", drawoffx, drawoffy, drawoffz, rotateAngleY);
		// rot x
    ...
    //I think one or more side texture mapping isn't quite right when there is rotation.... oh well...
    //
    public void renderToVBO(VBOBuffer v, StitchedTexture st, float red, float green, float blue, int xo, int yo, int zo, int roty)
    {
    	...
    	float ys = ye - sizeY;
    	//z is reversed?
    	float ze = -(rotationPointZ + offsetZ);
    	float zs = ze - sizeZ;   	
