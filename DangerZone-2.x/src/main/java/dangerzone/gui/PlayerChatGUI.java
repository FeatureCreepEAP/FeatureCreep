...
public class PlayerChatGUI extends GuiInterface {
	...
	public void process(){
		...
		int i;
		for(i=0;i<20;i++){
			if(!last20[i].equals(""))textAt(startx+50, bottom_of_display + ((i*30) + 100), last20[i]);
		}
		
		drawRectangleWithTexture(DangerZone.textinputtexture, startx + 115, bottom_of_display + 60, 500, 30);
		textAt(startx + 50, bottom_of_display + 60, "MSG:   " + currstring);
		...
		if(entered){
			...
			if(currstring.length() > 0){
				//Send it!
				DangerZone.server_connection.sendChatMessage(DangerZone.playername + ": " + currstring);
