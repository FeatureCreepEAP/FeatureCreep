...
public class ServerHooker {
	...
	public static void onBlockBroken(Player p, int d, int x, int y, int z, int s){
		if(ServerHooksArray == null)return;		
		//now custom server checks
		for(int i=0;i<ServerHooksMAX;i++){
			if(ServerHooksArray[i] != null){
				ServerHooksArray[i].onBlockBroken(p, d, x, y, z, s);
			}
		}
	}
	public static void leftClickOnBlock(Player p, int d, int x, int y, int z, int s){
		if(ServerHooksArray == null)return;		
		//now custom server checks
		for(int i=0;i<ServerHooksMAX;i++){
			if(ServerHooksArray[i] != null){
				ServerHooksArray[i].leftClickOnBlock(p, d, x, y, z, s);
			}
		}
	}
	public static void rightClickOnBlock(Player p, int d, int x, int y, int z, int s){
		if(ServerHooksArray == null)return;		
		//now custom server checks
		for(int i=0;i<ServerHooksMAX;i++){
			if(ServerHooksArray[i] != null){
				ServerHooksArray[i].rightClickOnBlock(p, d, x, y, z, s);
			}
		}
	}
