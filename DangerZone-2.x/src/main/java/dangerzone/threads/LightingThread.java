		...
		while(DangerZone.gameover == 0){
			...
			for(i=-tickmax;i<=tickmax && DangerZone.gameover == 0 && !restart;i++){
				for(j=-tickmax;j<=tickmax && DangerZone.gameover == 0 && !restart;j++){ 
					...
					chance = i*i + j*j;
					if(Math.sqrt(chance) > DangerZone.renderdistance/4 && DangerZone.world.rand.nextInt(chance+2) != 0)continue; //Do closer more often, further less often
				...
				for(i=0;i<16;i++){
					for(k=0;k<16;k++){
						if(lightmap[i*16+k] > 19){
							lightmap[i*16+k] -= 20;
							if(lightmap[i*16+k] > 19)active = true;
