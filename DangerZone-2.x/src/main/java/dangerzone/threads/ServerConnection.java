...
public class ServerConnection implements Runnable {
	...
	public void run()  {
		...
		try {
			sock.setReceiveBufferSize(1024*128);
		...
		try {
			sock.setSendBufferSize(1024*64);
		} catch (SocketException e2) {
			// TODO Auto-generated catch block
			e2.printStackTrace();
		}
		...
		try {
			...
			objectInput = new NetworkInputBuffer(sock.getInputStream(), 1024*128);
			objectOutput = new NetworkOutputBuffer(sock.getOutputStream(), 256); //packet count, not bytes!
		...
		try {		
			...
			while(DangerZone.gameover == 0){	
				...
				DangerZone.packets_per_second++; //Client side counter! easy!
				...
				if(packettype == PacketTypes.SPAWNPARTICLESSCALED){ 
					...
					id = objectInput.readInt();
					fpx = objectInput.readFloat();
					Utils.spawnParticlesScaled(DangerZone.player.world, s, which, d, px, py, pz, id, fpx, false); //spawn 'em, and DO NOT forward back to server
					continue;
				}
				...
				if(packettype == PacketTypes.SPAWNPARTICLESSCALEDWITHDIRECTION){ 
					...
					id = objectInput.readInt();
					fpx = objectInput.readFloat();
					fpy = objectInput.readFloat(); //base velocity
					fpz = objectInput.readFloat(); //randomness
					Utils.spawnParticlesScaledWithDirection(DangerZone.player.world, s, which, d, px, py, pz, mx, my, mz, id, fpx, fpy, fpz, false); //spawn 'em, and DO NOT forward back to server
					continue;
				}
				...
				//-----------------------------------------------------Server telling us to back down!
				if(packettype == PacketTypes.RENDERDISTANCECHANGE){ 
					d = objectInput.readInt();
					if(d >= 8 && d <= 24) {
