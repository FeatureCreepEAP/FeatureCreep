...
public class VBODataBuilderThread implements Runnable {
	...
	private boolean renderChunk(World world, int xrel, int zrel, int torender, int shouldcheck, int renderlevel, int pd, double px, double py, double pz){
		...
		if(!defective && dist > 2){
			if(c.VBOids[0] > 0){
				if(renderlevel < 2){
					if(world.rand.nextInt((int)(dist/2) + 1) != 0){
						if(currtime - c.last_render_time < 8000){ //over 8 seconds, redraw it anyway. Keeps lighting more consistent!
							return false;
						}
						shouldcheck = 1;
					}
