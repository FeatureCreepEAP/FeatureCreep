	...
	public void tickMe(World w, int d, int x, int y, int z){
		...
		if(w.rand.nextInt(25) == 1){
			for(i=-2;i<=2;i++){
				for(j=-1;j<=1;j++){
					for(k=-2;k<=2;k++){
						if(w.getblock(d, x+i, y+j, z+k)==blockID){
							count++;
						}
					}
				}
			...
			if(count > 3){
				w.setblock(d, x, y, z, Blocks.water.blockID);
				//System.out.printf("Grass died 3\n");
				return;
			}
		}
