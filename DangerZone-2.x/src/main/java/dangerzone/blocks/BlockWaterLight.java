 ...
 * 
 */
public class BlockWaterLight extends Block {
	...
	public void tickMe(World w, int d, int x, int y, int z){	
		...
		if(spout){
			w.setblockandmeta(d, x, y, z, blockID, 0x01);
			if(brightness > 0){
				Utils.spawnParticlesFromServer(w, "DangerZone:ParticleSparkle", 1 + w.rand.nextInt(10), d, (double)x+0.5f, (double)y+1.1f, (double)z+0.5f);
			}else{
				Utils.spawnParticlesFromServer(w, "DangerZone:ParticleSmoke", 1 + w.rand.nextInt(10), d, (double)x+0.5f, (double)y+1.1f, (double)z+0.5f);
			}
		}else{
			w.setblockandmeta(d, x, y, z, blockID, 0);
		}
	}
