...
public class BlockFurnace extends Block {
	...
	public BlockFurnace(String n){
		super(n, "");
		topname = "res/blocks/furnace_top.png";
		bottomname = "res/blocks/furnace_bottom.png";
		leftname = "res/blocks/furnace_left.png";
		rightname = "res/blocks/furnace_right.png";
		frontname = "res/blocks/furnace_front.png";
		backname = "res/blocks/furnace_back.png";
		maxstack = 8;
		isStone = true;
		hasFront = true;
		renderAllSides = true;
	...
	public boolean rightClickOnBlock(Player p, int dimension, int x, int y, int z, int s){
		...
		if(ec == null){ //where did our entity go???
			...
			Entity eb = p.world.createEntityByName("DangerZone:EntityFurnace", dimension, ((double)x)+0.5f, ((double)y)+0.05f, ((double)z)+0.5f);
			if(eb != null){
				eb.init();
		...
		if(!p.world.isServer){
			...
			DangerZone.setActiveGui(DangerZone.furnacegui);
		}
	...
	public void onBlockPlaced(World w, int dimension, int x, int y, int z){
		if(w.isServer){
			...
			Entity eb = w.createEntityByName("DangerZone:EntityFurnace", dimension, ((double)x)+0.5f, ((double)y)+0.05f, ((double)z)+0.5f);
			if(eb != null){
				eb.init();
				eb.setBID(Blocks.furnace.blockID);
				eb.setIID(Blocks.furnaceOn.blockID);
