...
public class EntityLiving extends Entity {
	...
	public boolean enable_buddy_nearest = false;
	public int findbuddydistance = 16;
	public int findbuddyfrequency = 30;
	...
	public boolean isDying(){
		if(deathfactor > 0 && deathfactor < 5.0f)return true;
		return false;
	}
	
	public void doDeathAnimation(){
		deathfactor = 1.01f; //start
	...
	public void update(float deltaT){
		...
		if(world.isServer && getCanDespawn()){
			if((world.isDaytime() && daytimedespawn) || (!world.isDaytime() && nighttimedespawn)){
				...
				if(p == null){
					...
					int ratemul = 1;
					if(getAttacking())ratemul = 10; //busy, stick around!
					...
					if(dist > DangerZone.entityupdatedist){
						...
						if(world.rand.nextInt(1000*ratemul) == 0){
							deadflag = true;
						}
					}else if(dist > DangerZone.entityupdatedist/8){
						if(world.rand.nextInt(10000*ratemul) == 0){
							deadflag = true;
						}
					}
				}
		...
		if(deadflag)deathfactor += 0.12f; //Delta death animation expander
		...
		if(world.isServer){		
			...
			if(!(this instanceof Player)){
					...
					for(i=0;i>=idf;i--){
						if(isSolidAtLevel(dimension, posx, posy+df+i, posz)){ 
							...
							if(fallcount > 7){
								...
								Utils.spawnParticlesFromServerScaled(world, "DangerZone:ParticleBreakStep", 100, this.dimension, posx, posy, posz, 
										world.getblock(dimension, (int)posx, (int)(posy+df+i), (int)posz), 0.25f*(getWidth()+getHeight()));
								Utils.spawnParticlesFromServerScaled(world, "DangerZone:ParticleDust", 100, this.dimension, posx, posy, posz, 
										0, 0.25f*(getWidth()+getHeight()));
								world.playSound("DangerZone:big_thud", dimension, posx, posy, posz, 0.75f, 1.0f+((world.rand.nextFloat()-world.rand.nextFloat())*0.1f));
							}
							fallcount = 0;
							break;
						}
				...
				if(!isSolidAtLevel(dimension, posx, posy-0.15f, posz) && getRiddenEntity() == null){
					...
					if(!getOnGround()){
						Utils.spawnParticlesFromServerScaled(world, "DangerZone:ParticleBreakStep", 50, this.dimension, posx, posy, posz, bid, 0.25f*(getWidth()+getHeight())/2);
						Utils.spawnParticlesFromServerScaled(world, "DangerZone:ParticleDust", 20, this.dimension, posx, posy, posz, 0, 0.25f*(getWidth()+getHeight())/2);
						world.playSound("DangerZone:small_thud", dimension, posx, posy, posz, 0.05f, 1.0f+((world.rand.nextFloat()-world.rand.nextFloat())*0.1f));
					}
					setOnGround(true);
					fallcount = 0;	
					
					//motion dust! Sadly, just always true!
					df = (float) Math.sqrt(motionx*motionx + motionz*motionz);
					if(DangerZone.movepart && df > moveSpeed*9/10 && bid > 0){
						Utils.spawnParticlesFromServerScaled(world, "DangerZone:ParticleBreakStep", 5, this.dimension, posx, posy, posz, bid, 0.25f*(getWidth()+getHeight())/2);
						Utils.spawnParticlesFromServerScaled(world, "DangerZone:ParticleDust", 2, this.dimension, posx, posy, posz, 0, 0.25f*(getWidth()+getHeight())/2);
					}
				...
				*/
				...
				if(!isSolidAtLevel(dimension, posx, posy-0.10f, posz) && getRiddenEntity() == null){
					...
					if(!getOnGround()){
						Utils.spawnParticlesFromServerScaled(world, "DangerZone:ParticleBreakStep", 50, this.dimension, posx, posy, posz, bid, 0.25f*(getWidth()+getHeight())/2);
						Utils.spawnParticlesFromServerScaled(world, "DangerZone:ParticleDust", 20, this.dimension, posx, posy, posz, 0, 0.25f*(getWidth()+getHeight())/2);
						//do Sound from client. The delay sounds too awful!
						//world.playSound("DangerZone:small_thud", dimension, posx, posy, posz, 0.05f, 1.0f+((world.rand.nextFloat()-world.rand.nextFloat())*0.1f));
					}
					setOnGround(true);
					fallcount = 0;
					...
					//motion dust!
					float ddf = (float) Math.sqrt(motionx*motionx + motionz*motionz);
					if(DangerZone.movepart && ddf > moveSpeed*9/10 && bid > 0){
						Utils.spawnParticlesFromServerScaled(world, "DangerZone:ParticleBreakStep", 5, this.dimension, posx, posy, posz, bid, 0.25f*(getWidth()+getHeight())/2);
						Utils.spawnParticlesFromServerScaled(world, "DangerZone:ParticleDust", 2, this.dimension, posx, posy, posz, 0, 0.25f*(getWidth()+getHeight())/2);
					}
	...
	public void doAttackFromCustom(/*entity that hit me*/Entity e, /*DamageTypes*/int dt, float pain, boolean addknockback){	
		...
		if(DangerZone.petprotection){
			if(getOwnerName() != null){
				if(dt != DamageTypes.ABSOLUTE)return;
			}
		}
		...
		if(dt != DamageTypes.ABSOLUTE){
			...
			totalpain /= dfn; //Divide intended damage by total defense
		...
		if(totalpain > 0){	
			...
			if(getHurtSound() != null && dt != DamageTypes.HUNGER && dt != DamageTypes.POISON){
				...
				Utils.spawnParticlesFromServer(this.world, "DangerZone:ParticleHurt", 50, this.dimension, posx, posy, posz);
				Utils.spawnParticlesFromServerScaled(world, "DangerZone:ParticleHurt", 50, this.dimension, posx, posy, posz, 
						0, 0.25f*(getWidth()+getHeight())/2);
			}
	...
	public void do_breeding(){
		...
		if(breeding != 0){
			if(world.rand.nextInt(5) == 0)Utils.spawnParticlesFromServer(world, "DangerZone:ParticleHappy", 3, dimension, posx, posy+getHeight(), posz);
		}
	}
	...
	public void doFindBuddy(){
		if(world.rand.nextInt(findbuddyfrequency) == 1){			
			List<Entity> nearby_list = null;
			int nbuddy = 0;
			double totalx, totalz, bdist, curdist;
			totalx = totalz = 0;
			curdist = 0;
			bdist = 9999;
			...
			if(nearby_list != null){
				if(!nearby_list.isEmpty()){
					...
					while(li.hasNext()){
						e = (Entity)li.next();												
						if(e != this && isBuddy(e)){
							if(enable_buddy_nearest){
								curdist = getDistanceFromEntity(e);
								if(curdist < bdist){
									bdist = curdist;
									totalx = e.posx;
									totalz = e.posz;
			...
			if(world.isServer){
					...
					if(enable_taming){	
						...
						if(ic != null && isFoodForMe(ic.bid, ic.iid) && getOwnerName() == null){
							if(world.rand.nextInt(3) == 1){
								...
								Utils.spawnParticlesFromServer(world, "DangerZone:ParticleHappy", 30, dimension, posx, posy+getHeight(), posz);
								heal(10);
								setCanDespawn(false);
								if(temperament == Temperament.HOSTILE)temperament = Temperament.PASSIVE_AGGRESSIVE;
							}
				...
				if(enable_taming){	
					...
					if(ic != null && ic.iid == Items.vampireteeth.itemID && getOwnerName() != null && p.myname.equals(getOwnerName())){
						Utils.spawnParticlesFromServer(world, "DangerZone:ParticleSmoke", 20, dimension, posx, posy+getHeight(), posz);
						setStaying(false);
						setOwnerName(null);
						setPetName(null);
						setCanDespawn(true);
					...
					if(ic != null && getOwnerName() != null && p.myname.equals(getOwnerName()) && getRiderEntity() == null){
						if(getStaying()){
							...
							motionx = motionz = 0;
							target = null;
							Utils.spawnParticlesFromServer(world, "DangerZone:ParticleSmoke", 30, dimension, posx, posy+getHeight(), posz);
						}
					}
