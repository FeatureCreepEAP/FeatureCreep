	...
	public void doEntityAction(float deltaT){	
		...
		{	
			...
			while(bid != Blocks.waterstatic.blockID && keep_trying != 0){
				...
				bid = this.world.getblock(this.dimension, (int)this.target.targetx, (int)(this.target.targety+0.5f), (int)this.target.targetz);
				if(bid == Blocks.waterstatic.blockID){
		...
		if(target != null){
			...
			motiony += 0.075f * deltaT; //Counter a little gravity!
			dy = dy/20f;
			...
			if(getInLiquid())Utils.SpawnBubblesAlongPath(this);
