...
public class Entity {
	...
	public float getRiderYoffset(){
		return getHeight(); //This is most assuredly wrong... override for your rideable critter!
	}
	...
	public void setRightButtonDownCount(int count){ 
		if(!world.isServer && this == DangerZone.player){
			//need to update this to server so other people can see what we are doing!!!
			if(count != getVarInt(15))DangerZone.player.server_connection.sendVarIntMessage(entityID, 15, count);
		}
		setVarInt(15, count);
	}
	...
	//float 8 is free. used to be setRightButtonDownCount
	...
	public void update(float deltaT){
		...
		if(world.isServer){
			if(this instanceof EntityLiving){
				...
				if(posy < -64)me.doAttackFromCustom(null, DamageTypes.ABSOLUTE, me.getMaxHealth()/20, false);
				if(posy < -1024)deadflag = true; //failsafe
				if(posx < 0 || posz < 0)me.doAttackFromCustom(null, DamageTypes.ABSOLUTE, me.getMaxHealth()/20, false);
				if(posx > Integer.MAX_VALUE || posz > Integer.MAX_VALUE)me.doAttackFromCustom(null, DamageTypes.ABSOLUTE, me.getMaxHealth()/20, false);
			}else{
