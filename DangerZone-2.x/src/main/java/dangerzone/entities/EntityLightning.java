	...
	public void update(float deltaT){
		...
		if(world.isServer){
			...
			if(countdown == 1){
				...
				world.playSound(thunder, dimension, posx, posy, posz, vol, 1.0f);
				...
				while(li.hasNext()){
					...
					if(enthit != this && !enthit.canthitme && enthit instanceof EntityLiving){ //don't hit self or things that shouldn't be hit!
						if(enthit.posy+enthit.getHeight() > posy-3 && enthit.posy < posy+howhigh){
