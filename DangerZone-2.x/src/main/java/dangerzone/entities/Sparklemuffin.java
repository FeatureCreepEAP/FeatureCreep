...
public class Sparklemuffin extends EntityLiving {
	...
	public	Sparklemuffin(World w){
		...
		this.height = 0.65f;
		this.width = 1.25f;
		uniquename = "DangerZone:Sparklemuffin";
		moveSpeed = 0.35f;
		setMaxHealth(100.0f);
		setHealth(100.0f);
		setDefense(2.0f);
	...
	public void doDeathDrops(){
		if(world.rand.nextInt(10)==1)Utils.doDropRand(world, 0, Items.trophysparklemuffin.itemID, 1f, dimension, posx, posy, posz);
	...
	public void doAttack(Entity victim){
		Effects ef = new Effects(Effects.CONFUSION, 1.5f, 100);
	...
	public Texture getTexture(){
		if(texture == null){
			...
			texture = TextureMapper.getTexture("res/skins/"+ "Sparklemuffintexture.png");	//this is not fast, so we keep our own pointer!
		}
		return texture;
	}
	...
	public boolean isSuitableTarget(Entity e){
		...
		if(e instanceof Sparklemuffin)return false;
