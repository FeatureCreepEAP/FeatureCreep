...
public class WorldRenderer {
	...
	public static volatile double poi_x, poi_y, poi_z; //point of impact!!! Client side only.
		...
		if(hit_cycle_count != 0){
			...
			if(hit_cycle_count >= hit_cycles){
				if(DangerZone.doleftclick){
					DangerZone.player.leftclick(world, usex, usey, usez, uses, usee==null?0:usee.entityID, DangerZone.magic_power, DangerZone.magic_type, poi_x, poi_y, poi_z);
					DangerZone.doleftclick = false;
					DangerZone.magic_power = 0;
				}
				if(DangerZone.dorightclick){
					DangerZone.player.rightclick(world, usex, usey, usez, uses, usee==null?0:usee.entityID, poi_x, poi_y, poi_z);
				...
				if(ic.iid != 0){
					...
					if(Items.isFlipped(ic.iid)){
						if(Items.getfullholdcount(ic.iid)!=0 && (DangerZone.rightbuttondowncounter != 0 || DangerZone.rapidfire_delay != 0)){								
							...
							GL11.glRotatef(-174.2f, 1.0f, 0.0f, 0.0f); // Rotate The Entity On X, Y & Z
							GL11.glRotatef(65.3f+(bounce*4), 0.0f, 1.0f, 0.0f);
							GL11.glRotatef(-39.2f+(bounce*4), 0.0f, 0.0f, 1.0f);
						}else if(hit_cycle_count != 0){														
								GL11.glTranslatef(1+hitx*((float)hit_cycle_count/hit_cycles), 3+hity*((float)hit_cycle_count/hit_cycles), 3+hitz*((float)hit_cycle_count/hit_cycles));
								GL11.glRotatef(-91+hitp*((float)hit_cycle_count/hit_cycles), 1.0f, 0.0f, 0.0f); // Rotate The Entity On X, Y & Z
								GL11.glRotatef(102+(bounce*4)+hitw*((float)hit_cycle_count/hit_cycles), 0.0f, 1.0f, 0.0f);
								GL11.glRotatef(-124+(bounce*4)+hitr*((float)hit_cycle_count/hit_cycles), 0.0f, 0.0f, 1.0f);
							...
							GL11.glTranslatef(1+eatx*((float)eat_cycle_count/eat_cycles), 3+eaty*((float)eat_cycle_count/eat_cycles), 3+eatz*((float)eat_cycle_count/eat_cycles));
							GL11.glRotatef(-91+eatp*((float)eat_cycle_count/eat_cycles), 1.0f, 0.0f, 0.0f); // Rotate The Entity On X, Y & Z
							GL11.glRotatef(102+(eatbounce*4)+eatw*((float)eat_cycle_count/eat_cycles), 0.0f, 1.0f, 0.0f);
							GL11.glRotatef(-124+(eatbounce*4)+eatr*((float)eat_cycle_count/eat_cycles), 0.0f, 0.0f, 1.0f);
						}else{
							GL11.glTranslatef(1, 3, 3);
							GL11.glRotatef(-91, 1.0f, 0.0f, 0.0f); // Rotate The Entity On X, Y & Z
							GL11.glRotatef(102+(bounce*4), 0.0f, 1.0f, 0.0f);
							GL11.glRotatef(-124+(bounce*4), 0.0f, 0.0f, 1.0f);
						}
					}else{
						//other way
						if(Items.getfullholdcount(ic.iid)!=0 && (DangerZone.rightbuttondowncounter != 0 || DangerZone.rapidfire_delay != 0)){							
							GL11.glTranslatef(-7.7f, 8.3f, 5.4f);
							GL11.glRotatef(-6.3f, 1.0f, 0.0f, 0.0f); // Rotate The Entity On X, Y & Z
							GL11.glRotatef(-60.1f+(bounce*4), 0.0f, 1.0f, 0.0f);
							GL11.glRotatef(120.5f+(bounce*4), 0.0f, 0.0f, 1.0f);
						}else if(hit_cycle_count != 0){
							GL11.glTranslatef(1.2f+hitxn*((float)hit_cycle_count/hit_cycles), 4.0f +hityn*((float)hit_cycle_count/hit_cycles), 0.1f+hitzn*((float)hit_cycle_count/hit_cycles));
							GL11.glRotatef(-30f+hitpn*((float)hit_cycle_count/hit_cycles), 1.0f, 0.0f, 0.0f); // Rotate The Entity On X, Y & Z
							GL11.glRotatef(-67.7f+(bounce*4)+hitwn*((float)hit_cycle_count/hit_cycles), 0.0f, 1.0f, 0.0f);
							GL11.glRotatef(120.5f+(bounce*4)+hitrn*((float)hit_cycle_count/hit_cycles), 0.0f, 0.0f, 1.0f);
						}else if(eat_cycle_count != 0){
							eatbounce = bounce;
							if(eat_cycle_count >= eat_cycles){						
								if(eat_delay_count > 0){
							...
							GL11.glTranslatef(1.2f+eatxn*((float)eat_cycle_count/eat_cycles), 4.0f +eatyn*((float)eat_cycle_count/eat_cycles), 0.1f+eatzn*((float)eat_cycle_count/eat_cycles));
							GL11.glRotatef(-30f+eatpn*((float)eat_cycle_count/eat_cycles), 1.0f, 0.0f, 0.0f); // Rotate The Entity On X, Y & Z
							GL11.glRotatef(-67.7f+(eatbounce*4)+eatwn*((float)eat_cycle_count/eat_cycles), 0.0f, 1.0f, 0.0f);
							GL11.glRotatef(120.5f+(eatbounce*4)+eatrn*((float)eat_cycle_count/eat_cycles), 0.0f, 0.0f, 1.0f);
						}else{
							GL11.glTranslatef(1.2f, 4.0f , 0.1f);
							if(Items.isFlopped(ic.iid)){
								GL11.glRotatef(-6f, 1.0f, 0.0f, 0.0f); // Rotate The Entity On X, Y & Z
							}else{
								GL11.glRotatef(-30f, 1.0f, 0.0f, 0.0f); // Rotate The Entity On X, Y & Z
							}
								
							GL11.glRotatef(-67.7f+(bounce*4), 0.0f, 1.0f, 0.0f);
							GL11.glRotatef(120.5f+(bounce*4), 0.0f, 0.0f, 1.0f);
					...
					if(dist <= ent.maxrenderdist && dist <= torender*16 ){
						...
						if(ent.model != null){	
							...
							if(washurt && DangerZone.showhitbox){
								if(ent.isBaby())GL11.glScalef(0.25f, 0.25f, 0.25f);
								GL11.glTranslated(0, ent.getHeight()*8, 0);
								GL11.glScalef(ent.getWidth(), ent.getHeight(), ent.getWidth());
								...
								GL11.glScalef(1f/ent.getWidth(), 1f/ent.getHeight(), 1f/ent.getWidth());	
								GL11.glTranslated(0, -ent.getHeight()*8, 0);
								if(ent.isBaby())GL11.glScalef(4, 4, 4);
							}
