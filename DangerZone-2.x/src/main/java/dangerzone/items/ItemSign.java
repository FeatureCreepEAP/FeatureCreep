...
public class ItemSign extends Item {
	...
	public ItemSign(String name, String tx){
		super(name, tx);
	}
	...
	public boolean rightClickOnBlock(Player p, int dimension, int x, int y, int z, int side){
		if(p != null && p.world.isServer && side == 0){
			if(!BreakChecks.canChangeBlock(p, dimension, x, y, z, 0, 0))return false;
			Entity eb = p.world.createEntityByName("DangerZone:EntitySign", dimension, ((double)x)+0.5f, ((double)y)+1.05f, ((double)z)+0.5f);
			if(eb != null){
				eb.init();
