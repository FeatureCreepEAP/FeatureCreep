...
public class ItemCorn extends ItemFood {
	...
	public ItemCorn(String name, String tx){
		super(name, tx, 8);
		maxstack = 64;
		burntime = 45;
	}
		...
		if(p != null && p.world.isServer && side == 0){
			...
			if(bid == Blocks.dirt.blockID){
				if(p.world.getblock(dimension, x, y+1, z) == 0){
					p.world.setblockandmetaWithPerm(p, dimension, x, y+1, z, Blocks.corn_plant.blockID, p.world.rand.nextInt(4)+4); //set block and plant height
					return true;
				}
			}
		}
