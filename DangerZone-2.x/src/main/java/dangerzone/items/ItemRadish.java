 ...
 * 
 */
public class ItemRadish extends ItemFood {
	
	
	public ItemRadish(String name, String tx){
		super(name, tx, 3);
		maxstack = 64;
		burntime = 15;
	}
		...
		if(p != null && p.world.isServer && side == 0){
			...
			if(bid == Blocks.dirt.blockID){
				if(p.world.getblock(dimension, x, y+1, z) == 0){
					p.world.setblockandmetaWithPerm(p, dimension, x, y+1, z, Blocks.radish_plant.blockID, 0);
					return true; //decrement item count
				}
			}
		}
		return false;
