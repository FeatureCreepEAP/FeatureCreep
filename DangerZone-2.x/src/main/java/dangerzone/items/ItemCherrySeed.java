 ...
 * 
 */
public class ItemCherrySeed extends Item {
	
	
	public ItemCherrySeed(String name, String tx){
		super(name, tx);
		maxstack = 128;
		burntime = 2;
	}
		...
		if(p != null && p.world.isServer && side == 0){
			...
			if(bid == Blocks.dirt.blockID || bid == Blocks.grassblock.blockID){
				if(p.world.getblock(dimension, x, y+1, z) == 0){
					Trees tr = new Trees();
					tr.makeFruitTree(p.world, dimension, x, y, z, Blocks.cherryleaves.blockID);
