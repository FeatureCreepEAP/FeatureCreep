...
public class RubyOre extends Block implements IHadModels, IMetaName{
	...
	public RubyOre(String name, String dimension) {
		super(Material.IRON);
		// TODO Auto-generated constructor stub
	setUnlocalizedName(name);
	setRegistryName(name);
	//setCreativeTab(CreativeTabs.BUILDING_BLOCKS);
	setDefaultState(this.blockState.getBaseState().withProperty(VARIANT, EnumHandler0.EnumType.RUBY));
	
	this.name = name;
	this.dimension = dimension;
	
	FeatureCreepMC.BLOCKS.add(this);
	FeatureCreepMC.ITEMS.add(new ItemBlockVariants(this).setRegistryName(this.getRegistryName()));
	}
	...
	for(int i = 0; i < EnumHandler0.EnumType.values().length; i++)
	{
		...
		FeatureCreepMC.proxy.registerVariantRenderer(Item.getItemFromBlock(this), i, "ore_" + this.dimension + "_" + EnumHandler0.EnumType.values()[i].getName(), "inventory");
	}
