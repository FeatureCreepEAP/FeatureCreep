...
class LookingGlass implements ClassProvider {
    ...
    MethodAccessor<Return> createAccessor(Thing target, Method method) {
       ...
       // final Class<?> point = target instanceof Class c ? c : target.getClass(); We may need to reimplement this
        final Class<?> point = target.getClass(); // this may have issues
        final Class<?> type;
        if (cache.containsKey(hash)) type = cache.get(hash);
        else create:{
    ...
    FieldAccessor<Type> createAccessor(Thing target, Field field) {
        ...
        if (cache.containsKey(hash)) type = cache.get(hash);
           ...
           // final Class<?> point = target instanceof Class c ? c : target.getClass(); Like other class get targets
