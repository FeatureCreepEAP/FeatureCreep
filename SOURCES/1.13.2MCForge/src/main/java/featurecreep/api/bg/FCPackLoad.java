...
import game.ResourcePackInfo.Factory;
...
public class FCPackLoad implements ResourcePackProvider {
	...
	@Override
	public <T extends ResourcePackInfo> void register(Map<String, T> registry, Factory<T> factory) {
		...
		for (Entry<String, VainillaResourcePack> entry : PackLoader.packs.entrySet()) {
			...
			if (!isNative(pack) && !pack.isEmpty()) {
				T info = ResourcePackInfo.createResourcePack(pack.getPackName(), true, () -> pack, factory,
						InsertionPosition.TOP);
				if (info != null) {
					System.out.println("Adding FCDatapack");
					registry.put(pack.getPackName(), info);
				}
