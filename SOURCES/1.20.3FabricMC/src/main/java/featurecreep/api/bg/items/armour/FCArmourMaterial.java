...
public  int getFCTextureNumber(FCArmourSlot slot)
{
	...
	public int getDurability(Type var1) {
		...
		//Same for all for now, may change later
		return durability;
	}

	@Override //They be copying FC
	public int getProtection(Type var1) {
		// TODO Auto-generated method stub
		if (var1.equals(Type.BOOTS)) {return getFCProtection(FCArmourSlots.BOOTS);}
		if (var1.equals(Type.CHESTPLATE)) {return getFCProtection(FCArmourSlots.TUBIC);}
		if (var1.equals(Type.LEGGINGS)) {return getFCProtection(FCArmourSlots.LEGGINGS);}
		if (var1.equals(Type.HELMET)) {return getFCProtection(FCArmourSlots.HELMET);}
		return 0;
	}
