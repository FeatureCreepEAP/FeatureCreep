...
class LookingGlass implements ClassProvider {
    ...
    static boolean isReachable(Object thing) {
        if (thing instanceof Class<?>) { // if needed replace most thing with object in below code
        	final Class<?> clazz = (Class)thing;
        	return Modifier.isPublic(clazz.getModifiers()); //&& LookingGlass.class.getModule().canRead(clazz.getModule());
        }
        else if (thing instanceof Method)
        {
        	Method meth = (Method)thing; 
            return Modifier.isPublic(meth.getModifiers()) && isReachable(meth.getDeclaringClass());
        }
        else if (thing instanceof Field) {
        	Field fiel = (Field)thing; 
            return Modifier.isPublic(fiel.getModifiers()) && isReachable(fiel.getDeclaringClass());
        }
        else if (thing instanceof Constructor)
        {
        	Constructor cons = (Constructor)thing; 
            return Modifier.isPublic(cons.getModifiers()) && isReachable(cons.getDeclaringClass());
        }
    ...
    ConstructorAccessor<Thing> createAccessor(Class<?> target, Constructor<?> constructor) {
        ...
        if (cache.containsKey(hash)) type = cache.get(hash);
            ...
            cache.put(hash, type);
            if (!(this.provider instanceof InternalAccessProvider)) break create;
            InternalAccessProvider prov = (InternalAccessProvider)this.provider;
            prov.assureAvailable(ConstructorAccessor.ConstructorAccessorImpl.class, target);
        }
    ...
    byte[] writeConstructorAccessor(Class<?> targetType, Constructor<?> constructor, String location) {
        final ClassWriter writer = new ClassWriter(ClassWriter.COMPUTE_MAXS);//ClassWriter(0);
        writer.visit(52, ACC_PUBLIC | ACC_SUPER, location, null, Type.getInternalName(ConstructorAccessor.ConstructorAccessorImpl.class), new String[]{Type.getInternalName(MethodAccessor.class)});
        constructor:
        {
    ...
    MethodAccessor<Return> createAccessor(Thing target, Method method) {
        ...
        Class<?> point = null;
        if (target instanceof Class){  point = (Class)target;}else point = target.getClass();
        final Class<?> type;
        if (cache.containsKey(hash)) type = cache.get(hash);
        else create:{
            ...
            if (!(this.provider instanceof InternalAccessProvider)) break create;
            InternalAccessProvider prov = (InternalAccessProvider)this.provider;
            prov.assureAvailable(MethodAccessor.MethodAccessorImpl.class, point);
        }
    ...
    byte[] writeMethodAccessor(Class<?> targetType, Method method, String location) {
        final ClassWriter writer = new ClassWriter(ClassWriter.COMPUTE_MAXS);//ClassWriter(0);
        writer.visit(52, ACC_PUBLIC | ACC_SUPER, location, null, Type.getInternalName(MethodAccessor.MethodAccessorImpl.class), new String[]{Type.getInternalName(MethodAccessor.class)});
        constructor:
        {
            final MethodVisitor visitor;
            visitor = writer.visitMethod(ACC_PUBLIC, "<init>", "(Ljava/lang/Object;)V", null, null);
    ...
    FieldAccessor<Type> createAccessor(Thing target, Field field) {
        ...
        if (cache.containsKey(hash)) type = cache.get(hash);
            ...
            if (this.provider instanceof InternalAccessProvider) {
            	final InternalAccessProvider prov = (InternalAccessProvider)this.provider;
                prov.assureAvailable(FieldAccessor.FieldAccessorImpl.class, point);
            }
    ...
    byte[] writeFieldAccessor(Class<?> targetType, Field field, String location) {
        final ClassWriter writer = new ClassWriter(ClassWriter.COMPUTE_MAXS);//ClassWriter(0);
        writer.visit(52, ACC_PUBLIC | ACC_SUPER, location, null, Type.getInternalName(FieldAccessor.FieldAccessorImpl.class), new String[]{Type.getInternalName(FieldAccessor.class)});
        constructor:
        {
            final MethodVisitor visitor;
            visitor = writer.visitMethod(ACC_PUBLIC, "<init>", "(Ljava/lang/Object;)V", null, null);
 ...
 //       } 
    ...
    <Template, Thing> Template makeInlineProxy(Mirror<Thing> mirror, Class<Template> template) {
        ...
        final String name;
        if (provider instanceof InternalAccessProvider) {
            name = template.getPackage().getName();
            InternalAccessProvider prov = (InternalAccessProvider)provider;
       //     prov.export(template.getModule(), name);
        } else {
    ...
    protected String getExportedPackageFrom(Class<?> place) {
      //  final Module module = place.getModule();
        ...
        if (place.getPackage().getName().startsWith("java.")) namespace = "jdk.internal.reflect"; // prohibited namespace
        else namespace = place.getPackage().getName();
        if (getProvider() instanceof InternalAccessProvider) {
        	final InternalAccessProvider prov = (InternalAccessProvider)provider;
          //  	prov.export(module, namespace);
        } 
        return namespace;
    }
    ...
    interface Handles {
        ...
        static Handle getHandle(final Method method) {
            ...
            return new Handle(code, Type.getInternalName(method.getDeclaringClass()), method.getName(), Type.getMethodDescriptor(method));
        }
        ...
        static Handle createHandle(String owner, String name, String descriptor) {
            return new Handle(H_INVOKESTATIC, owner, name, descriptor);
        }
