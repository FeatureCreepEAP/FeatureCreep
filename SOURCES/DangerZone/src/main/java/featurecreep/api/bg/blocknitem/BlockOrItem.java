...
public interface BlockOrItem<T> {
	...
	public default InventoryContainer toStack(int amount) {
		if (this instanceof FCItemAPI) {
			FCItemAPI item = (FCItemAPI) this;
			return new InventoryContainer(item.getFCRegistryName(), amount);
		} else {
			FCBlockAPI block = (FCBlockAPI) this;
			return new InventoryContainer(block.getFCRegistryName(), amount);
		}
