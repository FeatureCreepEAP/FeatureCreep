...
public class VanillaBlock implements FCBlockAPI<VanillaBlock> {
	...
	@Override
	public featurecreep.api.bg.blocks.BlockFieldHolder holder() {
		return holder;
	}

	public Block vanilla_block; // BE VERY CAREFUL
	// public IBlockState state; //BE EVEN MORE CAREFUL WITH THIS, MC SPECIFC TO NEW
	// VERSIONS
	...
	public VanillaBlock(Block block, String registry_name) {
		...
		setNumberID(block.blockID);// Will just be 0 in some versions most likely. Getting from States will be
									// weird though Gotta Maybe find a way to incorperate those
	}

	public VanillaBlock(FCBlockAPI block, String registry_name) // As a backup or for ported items
	{
		this(block.get(), registry_name);
	}
