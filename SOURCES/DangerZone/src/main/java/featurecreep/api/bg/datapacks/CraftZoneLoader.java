...
public class CraftZoneLoader {
	...
	public static void saveCraftNodes() {
		for (int i = 0; i < CraftingZone.objects.size(); i++) {
			String craftfile = new String("data/" + "craftingzone" + "/recipes/" + Math.random() + ".json");
			PackLoader.entries.put(craftfile,
					BasicIO.stringToByteArray(CraftingZone.objects.get(i).get119ModelNode().toJSONString(false)));
		}
	...
	public static void saveMeltNodes() {
		for (int i = 0; i < CraftingZone.melts.size(); i++) {
			String craftfile = new String("data/" + "craftingzone" + "/recipes/" + Math.random() + ".json");
			PackLoader.entries.put(craftfile,
					BasicIO.stringToByteArray(CraftingZone.melts.get(i).get119ModelNode().toJSONString(false)));
		}
