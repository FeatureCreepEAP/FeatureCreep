...
public class VanillaBlock implements FCBlockAPI<VanillaBlock>{
	...
	public VanillaBlock(Block block, IBlockstate state, String registry_name)
	{
		...
		setNumberID(block.serialise(state));//Will just be 0 in some versions most likely. Getting from States will be weird though Gotta Maybe find a way to incorperate those
	this.state = state;
	}
