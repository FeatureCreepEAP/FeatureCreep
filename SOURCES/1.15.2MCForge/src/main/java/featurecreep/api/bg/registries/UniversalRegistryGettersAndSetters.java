...
public class UniversalRegistryGettersAndSetters {
	...
	{
		FCForgeRegistries.ITEMS = new DeferredRegister(ForgeRegistries.ITEMS, registry_name.split(":")[0]);
	...
	public static void registerBlock(Block block, String registry_name, CreativeTab default_tab, int id)
	{
	    ...
	    ForgeRegistries.ITEMS.register(new BlockAsItem(block, new Item.Info().setCreativeTab(default_tab)).setRegistryName(registry_name));
	...
	if(registry.equals(GameRegistriesInterface.ITEM)) {
		ForgeRegistries.ITEMS.register((Item)Entry);
	}else if(registry.equals(GameRegistriesInterface.BLOCK)){
		ForgeRegistries.BLOCKS.register((Block)Entry);
	}else if(registry.equals(GameRegistriesInterface.ACTIVITY)){
			ForgeRegistries.ACTIVITIES.register((EntityActivity) Entry);
	}else if(registry.equals(GameRegistriesInterface.BLOCK_ENTITY)){
			ForgeRegistries.TILE_ENTITIES.register((TileEntites<?>) Entry);
	}else if(registry.equals(GameRegistriesInterface.CHUNK_STATUS)){
...
//		ForgeRegistries.BLOCK_STATE_PROVIDER_TYPES.register(rl, (Item)Entry);
		...
		public static DeferredRegister deferredRegistryConverter(GameRegistriesInterface registry, String name) {
			if(registry.equals(GameRegistriesInterface.ITEM)) {
				...
				return DeferredRegister.create(ForgeRegistries.TILE_ENTITIES, name);
			}else if(registry.equals(GameRegistriesInterface.CHUNK_STATUS)){
