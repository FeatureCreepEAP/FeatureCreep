...
public class FCLoaderBasicR6 implements FCLoaderBasic {
  ...
  public FCLoaderBasicR6(Path[] mod_locations, Path[] classpath_locations, String[] current_packages_exported, int threads, boolean debug_mode) { //We will probably add more variables son
...
System.out.println("Loading Runabble Mods");
  ...
  public void runMods() {
    // TODO Auto-generated method stub
	  ...
	  ExitManager securityManager = new ExitManager(System.getSecurityManager());
	  System.setSecurityManager( securityManager ) ;//Need to have a switch to turn this off
    ...
    System.setSecurityManager( securityManager.getOriginalSecurityManager() ) ;
  ...
  public ModuleLoader getBootModuleLoader() {
    ...
    return AccessController.doPrivileged(new PrivilegedAction < ModuleLoader > () {
      public ModuleLoader run() {
        ...
        try {
          return Class.forName(loaderClass, true, FCLoaderBasicR6.class.getClassLoader()).asSubclass(ModuleLoader.class).getConstructor().newInstance();
