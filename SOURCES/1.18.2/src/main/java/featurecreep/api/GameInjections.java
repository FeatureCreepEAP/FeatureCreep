...
public class GameInjections {
	...
	public static String game_version = "1.18.2";
	public static ActiveMapping mappings = ActiveMapping.OBF;// This is the default active mappings
	public static SuperLoader super_loader = SuperLoader.VANILLA_MINECRAFT;// Need to detect this eventually
...
public static int texture_pack_version = 8;
	...
	public static ByteBuffer inject(String nombre, ByteBuffer buff) {
		...
		} else if (nombre.equals(reverse_mappings.getClassMappedName("game.RegistryBootstrap"))) {
			return registryBootstrapInjection(buff);
		} else if (nombre.equals(reverse_mappings.getClassMappedName("game.SimpleRegistryMap"))) {
			return simpleRegistryMapInjection(buff);
		} else if (nombre.equals(reverse_mappings.getClassMappedName("game.ChunkGenerator"))) {
			return chunkGen(buff);
		} else if (nombre.equals(reverse_mappings.getClassMappedName("game.CommandDispatcher"))) {
			return commandDispatcherInjection(buff);
		}
	...
	public static ByteBuffer commandDispatcherInjection(ByteBuffer buff) {
		// TODO Auto-generated method stub

		try {
			ClassFile file = ClassFileUtils.classFileFromByteBuffer(buff);
			// String target =
			// reverse_mappings.getDefMappedName("game.CommandDispatcher.<init>(Lgame/CommandDispatcher$CommandSelection;)V");

			// initialise
			MethodInfo def = ClassFileUtils.getMethodInfoWithDescriptor(file, "<init>",
					reverse_mappings.renameClassesInMethodDescriptor("(Lgame/CommandDispatcher$CommandSelection;)V"));
			if (def != null) {
				CodeAttribute coat = def.getCodeAttribute();
				Bytecode code = new Bytecode(file.getConstPool());

				code.addGetfield(reverse_mappings.getClassMappedName("game.CommandDispatcher").replace(".", "/"),
						reverse_mappings.getVarMappedName(
								"game.CommandDispatcher.dispatcher:Lcom/mojang/brigadier/CommandDispatcher;"),
						"Lcom/mojang/brigadier/CommandDispatcher;");
				code.addInvokestatic("featurecreep/FeatureCreep", "registerFCDNF",
	...
	public static ByteBuffer chunkGen(ByteBuffer buff) {
		// TODO Auto-generated method stub

		try {
			ClassFile file = ClassFileUtils.classFileFromByteBuffer(buff);
			String target = reverse_mappings.getDefMappedName(
					"game.ChunkGenerator.generateFeatures(Lgame/StructureWorldGenerationAccessLevel;Lgame/IChunkAccess;Lgame/NudgerBuildings;)V");

			// initialise
			MethodInfo def = ClassFileUtils.getMethodInfoWithDescriptor(file, target,
					reverse_mappings.renameClassesInMethodDescriptor(
							"(Lgame/StructureWorldGenerationAccessLevel;Lgame/IChunkAccess;Lgame/NudgerBuildings;)V"));
			if (def != null) {
				CodeAttribute coat = def.getCodeAttribute();
				Bytecode code = new Bytecode(file.getConstPool());
				code.addAload(0);
				...
				code.addAload(2);
				code.addAload(3);
				code.addInvokestatic("featurecreep/api/orespawn/OrespawnBasicFeatureParser", "applyOres",
						reverse_mappings.renameClassesInMethodDescriptor(
								"(Lgame/ChunkGenerator;Lgame/StructureWorldGenerationAccessLevel;Lgame/Chunk;Lgame/NudgerBuildings;)V"));
				int len = coat.iterator().getCodeLength();
				coat.iterator().insert(len - 1, code.get());
	...
	public static ByteBuffer simpleRegistryMapInjection(ByteBuffer buff) {
		// TODO Auto-generated method stub
		try {
			ClassFile file = ClassFileUtils.classFileFromByteBuffer(buff);
			String target = reverse_mappings
					.getDefMappedName("game.SimpleRegistryMap.freeze()Lgame/GameRegistriesInterface;");
			MethodInfo def = ClassFileUtils.getMethodInfoWithDescriptor(file, target,
					reverse_mappings.getDefMappedName("()Lgame/GameRegistriesInterface;"));
			Bytecode nuevo = new Bytecode(def.getConstPool());
			nuevo.addAload(0);
			nuevo.addOpcode(Opcode.ARETURN);
			def.setCodeAttribute(def.getCodeAttribute());
			return ClassFileUtils.classFileToByteBuffer(file);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public static ByteBuffer registryBootstrapInjection(ByteBuffer buff) {
		// TODO Auto-generated method stub
		System.out.println("Adding FC Init to BootStrap");
		try {
			ClassFile file = ClassFileUtils.classFileFromByteBuffer(buff);
			String target = reverse_mappings.getDefMappedName("game.RegistryBootstrap.initialize()V");
			...
			if (def != null) {
				CodeAttribute coat = def.getCodeAttribute();
				Bytecode code = new Bytecode(file.getConstPool());
				code.addInvokestatic("featurecreep/FeatureCreep", "onInitialise", "()V");
				int len = coat.iterator().getCodeLength();
				coat.iterator().insert(len - 1, code.get());
