...
public class ModuleRemapper {
	...
	public static void remapMods() {
		...
		String debugpath = GameInjections.gamepath + "/etc/fcdebug";
		if (new File(debugpath).exists()) {
			GameInjections.debug_mode = true;
		}

		File temp_mapping_dir = new File(GameInjections.temp_mapping_location); // es obsoleto , solo existe ahora por que
					...
					if (mappings != null && "fci".equals(mappings)) {
						...
						for (String clazz : ClassPathUtils.getFilesFromJar(file)) {
							RemappingClassFileTransformer.fcied_classes.put(clazz.replace("/", "."),
									GameInjections.mappings.getMappings().getReverse());
						}
						...
						for (String clazz : ClassPathUtils.getFilesFromJar(file)) {
							RemappingClassFileTransformer.fcied_classes.put(clazz.replace("/", "."),
									GameInjections.mappings.getEnglishToNativeMappings().getReverse());
						}
					} // añadir más idiomas
				} else {
