...
public class UniversalRegistryGettersAndSetters {
	...
	public static FCItemAPI getFCItembyID(int id)
	{
		if (GlobalRegistries.getItemByID(id) != null)
		{
			...
			return new VanillaItem(GameRegistries.getItemFromGameRegistries(id), getRegistryName(GameRegistries.getItemFromGameRegistries(id)));
		}
	...
	{
		setRegistryName(item, registry_name);
		...
		ModelLoader.setCustomModelResourceLocation(item, 0, new gfx_asset(getRegistryName(item), "inventory"));	
	}
	...
	public static FCBlockAPI getFCBlockbyID(int id)
	{
		if (GlobalRegistries.getBlockByID(id) != null)
		{
			...
			return new VanillaBlock(GameRegistries.getBlockFromGameRegistries(id), getRegistryName(GameRegistries.getBlockFromGameRegistries(id)));
		}
	...
	{
		setRegistryName(block,registry_name);
		block.setUnlocalisedName(registry_name.split(":")[0] + "!" +  registry_name.split(":")[1]);
		block.setCreativeTab(default_tab);
		ForgeRegistries.BLOCKS.register(block);	
		BlockAsItem BAI = new BlockAsItem(block);
		setRegistryName(BAI,registry_name);
		ForgeRegistries.ITEMS.register(BAI);	
	...
	//VERSION SPECIFIC, remove later
	static void setRegistryName(Item item, String string) {
		try {
			item.getClass().getMethod("setRegistryName", String.class).invoke(item, string);
	...
	static void setRegistryName(Block item, String string) {
		try {
			item.getClass().getMethod("setRegistryName", String.class).invoke(item, string);
		} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException | NoSuchMethodException
				| SecurityException e) {
	...
	static String getRegistryName(Item item) {
		try {
			ResourceLocation rl = (ResourceLocation)item.getClass().getMethod("getRegistryName").invoke(item);
		return rl.toString();	
		} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException | NoSuchMethodException
				| SecurityException e) {
			// TODO Auto-generated catch block
	...
	static String getRegistryName(Block item) {
		try {
			ResourceLocation rl = (ResourceLocation)item.getClass().getMethod("getRegistryName").invoke(item);
			return rl.toString();
		} catch (IllegalAccessException | IllegalArgumentException | InvocationTargetException | NoSuchMethodException
