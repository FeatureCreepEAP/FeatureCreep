...
class IntrinsicMimicGenerator extends MimicGenerator {
    ...
    public <Template> Template createInline() {
        ...
        if (mirror.glass.provider instanceof InternalAccessProvider) { // We may need to put provider at the end of InternalAccessProvider as originally intended, though only is glass is used
            final Class<?> target = mirror.emergentClass();
            InternalAccessProvider prov = (InternalAccessProvider)mirror.glass.provider;
            prov.assureAvailable(top, target);
      //      prov.export(target.getModule(), target.getPackageName());
        }
    ...
    protected byte[] writeCode() {
        ...
        target:
        {
            final FieldVisitor visitor = writer.visitField(ACC_PUBLIC, "target", "L"+mirror.emergentClass().getCanonicalName().replace(".", "/")+";", null, null);
            visitor.visitEnd();
        }
        methods:
        {
    ...
    protected void writeCaller(Method method) {
        ...
        if (!Modifier.isStatic(target.getModifiers())) {
            ...
            visitor.visitFieldInsn(GETFIELD, internal, "target","L"+mirror.emergentClass().getCanonicalName().replace(".", "/")+";"); //"Ljava/lang/Class;"); //mirror.emergentClass().toString());  //mirror.emergentClass().descriptorString());
        }
        int index = 0;
    ...
    protected void writeFieldCaller(Method method) {
        ...
        if (method.getParameterCount() > 0) {
            if (!Modifier.isStatic(target.getModifiers())) {
                visitor.visitVarInsn(ALOAD, 0);
                visitor.visitFieldInsn(GETFIELD, internal, "target", "L"+mirror.emergentClass().getCanonicalName().replace(".", "/")+";");
            }
