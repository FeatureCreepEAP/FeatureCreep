...
public class CraftZoneLoader {
	...
	public static void saveCraftNodes()
	{
		for(int i=0; i<CraftingZone.objects.size(); i++) {
			File craftfile = new File(DataPackLoader.datapacklocation + "/assets/" + "craftingzone" + "/recipes/" + Math.random() + ".json");
			craftfile.getParentFile().mkdirs();
			try {
				FileWriter configwriter = new FileWriter(craftfile);
				configwriter.write(CraftingZone.objects.get(i).get112ModelNode().toJSONString(false));
				configwriter.close();
	...
	public static void saveMeltNodes()
	{
		for(int i=0; i<CraftingZone.melts.size(); i++) {
			...
			try {
				FileWriter configwriter = new FileWriter(craftfile);
				configwriter.write(CraftingZone.melts.get(i).get113ModelNode().toJSONString(false));
				configwriter.close();
