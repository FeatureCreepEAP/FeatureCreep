...
@Override
 ...
 public default void registerModels() {
	...
	public default void executeLeftClickOnBlock(AbstractPlayer p, FCWorld worl, FCBlockPos pos, FCBlockAPI block, int side) {
		toStack(1).getItem().onBlockLeftClick(toStack(1),worl.get(), block.get().defaultBlockstate(), pos, p.get());
	}
	...
	@Override
	public default void executeOnBlockBroken(AbstractEntity ent, FCBlockPos pos, FCBlockAPI block, int wasbid) {toStack(1).getItem().onBlockLeftClick(toStack(1), ent.getWorld().get(), block.get().defaultBlockstate(), pos, (LivingEntity)ent.get());};
