	...
	public void register(Consumer<ResourcePackInfo> consumer) {
		...
		for (Entry<String, VainillaResourcePack> entry : PackLoader.packs.entrySet()) {
			...
			if (!isNative(pack) && !pack.isEmpty()) {
				...
				ResourcePackInfo.PackMcMeta metadata = ResourcePackInfo.getPackMcMeta(pack.getPackName(), packFactory);
		        ResourcePackInfo info = ResourcePackInfo.createResourcePack(pack.getPackName(), instantTextBoxType.literal(pack.getPackName()), true, packFactory, metadata,ResourceType.CLIENT_RESOURCES, ResourcePackInfo.InsertionPosition.TOP, true, PackSources.BUILTIN);
				if (info != null) {
