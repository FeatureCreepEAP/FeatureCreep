...
public class VanillaBlock implements FCBlockAPI<VanillaBlock>{
	...
	public VanillaBlock(Block block, IBlockState state, String registry_name)
	{
		...
		setNumberID(block.getIdFromBlock(block));//Will just be 0 in some versions most likely. Getting from States will be weird though Gotta Maybe find a way to incorperate those
	this.state = state;
	}
