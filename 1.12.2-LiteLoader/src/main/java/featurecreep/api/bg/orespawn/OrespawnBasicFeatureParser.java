...
if (node.get("enabled").asBoolean() == true)
{
	...
	System.out.println(Block.REGISTRY.getNameForObject(replacedBlock));
	System.out.println(Block.REGISTRY.getNameForObject(newBlock));
...
public static void applyOres(World world, Random rand, BiomeDecorator dec)
{
	...
	//	accs.invoke(world, rand, configs.get(j).frequency, new WorldGenMinable(configs.get(j).block.getDefaultState(), configs.get(j).size), configs.get(j).minY, configs.get(j).maxY);
		...
		try {
		Method meth	= dec.getClass().getDeclaredMethod("a", World.class, Random.class, int.class, WorldGenerator.class, int.class, int.class);
	 meth.setAccessible(true);
	 meth.invoke(dec, world, rand, configs.get(j).frequency, new WorldGenMinable(configs.get(j).block.getDefaultState(), configs.get(j).size), configs.get(j).minY, configs.get(j).maxY);
